              ...
              <div class="col-md-4 stretch-card grid-margin">
                <div class="card bg-gradient-danger card-img-holder text-white">
                  <div class="card-body" onclick="myfunction()">
                    ...
                    <h4 class="font-weight-normal mb-3">Librarian <i class="float-right">{{ $LibrarianCount }}</i>
                    </h4>
                  </div>
                </div>
              </div>
                  ...
                  <div class="card">
                    <div class="card-body">
                      <table class="table">
                          ...
                          <tbody>
                            @if (count($LibrarianList) != 0)
                                @foreach ( $LibrarianList as $Librarian)
                                    <tr>
                                        <td>1</td>
                                        <td class="py-1">
                                            @if ($Librarian->profilePicture == null)
                                                <img src="../images/profile.png" alt="image" />
                                            @else
                                                <img src="../images/Librarian/{{ $Librarian->profilePicture }}" alt="image" />
                                            @endif
                                        </td>
                                        ...
                                        @else
                                            <td>Unregistered</td>
                                        @endif
                                        @if ($Librarian->isActive == 1)
                                            <td><a href="#open-modal-editactive-{{ $Librarian->id }}" class="badge badge-success">Active</a></td>
                                        @else
                                            <td><a href="#open-modal-editactive-{{ $Librarian->id }}" class="badge badge-danger">Inactive</a></td>
                                        @endif
                                    </tr>
                                @endforeach
                            @else
...
@foreach ( $LibrarianList as $Librarian)
    <form action="/admin/updatelibrarian" method="POST" enctype="multipart/form-data">
        @csrf
        <div id="open-modal-editactive-{{ $Librarian->id }}" class="modal-window">
            <div>
                ...
                <div class="input-field-pop">
                    <div class="white_box mb_30">
                        ...
                        <input type="hidden" name="id" id="id" value="{{ $Librarian->id }}">
                    </div>
                </div> <br>
